...
'''
    ...
    async def _generate_computer_use_script(
        ...
        if auto_detect_captcha:
            ...
            action_codes.append(f"                # 自动检测并处理验证码")
            action_codes.append(f"                try:")
            action_codes.append(f"                    # 尝试多种选择器查找验证码图片")
            action_codes.append(f"                    captcha_img = None")
            action_codes.append(f"                    captcha_selectors = [")
            action_codes.append(f"                        'img[src*=\"captcha\"]',")
            action_codes.append(f"                        'img[id*=\"captcha\"]',")
            action_codes.append(f"                        'img[alt*=\"captcha\"]',")
            action_codes.append(f"                        'img[alt*=\"验证码\"]',")
            action_codes.append(f"                        '.captcha img',")
            action_codes.append(f"                        '#captcha img',")
            action_codes.append(f"                        '[class*=\"captcha\"] img',")
            action_codes.append(f"                        'img[src*=\"code\"]',")
            action_codes.append(f"                        'img[src*=\"verify\"]'")
            action_codes.append(f"                    ]")
            action_codes.append(f"                    for selector in captcha_selectors:")
            action_codes.append(f"                        try:")
            action_codes.append(f"                            elements = page.locator(selector)")
            action_codes.append(f"                            if await elements.count() > 0:")
            action_codes.append(f"                                captcha_img = elements.first")
            action_codes.append(f"                                if await captcha_img.is_visible(timeout=1000):")
            action_codes.append(f"                                    print(f'使用选择器找到验证码: {{selector}}')")
            action_codes.append(f"                                    break")
            action_codes.append(f"                        except:")
            action_codes.append(f"                            continue")
            action_codes.append(f"                    ")
            action_codes.append(f"                    # 如果没找到，尝试查找所有图片并检查属性")
            action_codes.append(f"                    if not captcha_img:")
            action_codes.append(f"                        all_images = page.locator('img')")
            action_codes.append(f"                        for i in range(await all_images.count()):")
            action_codes.append(f"                            img = all_images.nth(i)")
            action_codes.append(f"                            try:")
            action_codes.append(f"                                src = await img.get_attribute('src') or ''")
            action_codes.append(f"                                alt = await img.get_attribute('alt') or ''")
            action_codes.append(f"                                if any(keyword in src.lower() or keyword in alt.lower() for keyword in ['captcha', '验证码', 'code', 'verify', 'check']):")
            action_codes.append(f"                                    captcha_img = img")
            action_codes.append(f"                                    if await captcha_img.is_visible(timeout=1000):")
            action_codes.append(f"                                        print(f'通过属性查找找到验证码: src={{src[:50]}}...')")
            action_codes.append(f"                                        break")
            action_codes.append(f"                            except:")
            action_codes.append(f"                                continue")
            action_codes.append(f"                    ")
            action_codes.append(f"                    if captcha_img and await captcha_img.is_visible(timeout=1000):")
            action_codes.append(f"                        print('检测到验证码')")
            action_codes.append(f"                        captcha_bytes = await captcha_img.screenshot()")
            action_codes.append(f"                        import base64")
            action_codes.append(f"                        captcha_base64 = base64.b64encode(captcha_bytes).decode('utf-8')")
            ...
            action_codes.append(f"                        print(f'识别到验证码: {{captcha_text}}')")
            action_codes.append(f"")
            action_codes.append(f"                        # 尝试多种选择器查找验证码输入框")
            action_codes.append(f"                        captcha_input = None")
            action_codes.append(f"                        input_selectors = [")
            action_codes.append(f"                            'input[name*=\"captcha\"]',")
            action_codes.append(f"                            'input[id*=\"captcha\"]',")
            action_codes.append(f"                            'input[placeholder*=\"captcha\"]',")
            action_codes.append(f"                            'input[placeholder*=\"验证码\"]',")
            action_codes.append(f"                            'input[name*=\"code\"]',")
            action_codes.append(f"                            'input[name*=\"verify\"]',")
            action_codes.append(f"                            'input[type=\"text\"][maxlength*=\"4\"]',")
            action_codes.append(f"                            'input[type=\"text\"][maxlength*=\"5\"]',")
            action_codes.append(f"                            'input[type=\"text\"][maxlength*=\"6\"]'")
            action_codes.append(f"                        ]")
            action_codes.append(f"                        for selector in input_selectors:")
            action_codes.append(f"                            try:")
            action_codes.append(f"                                elements = page.locator(selector)")
            action_codes.append(f"                                if await elements.count() > 0:")
            action_codes.append(f"                                    captcha_input = elements.first")
            action_codes.append(f"                                    if await captcha_input.is_visible(timeout=1000):")
            action_codes.append(f"                                        print(f'使用选择器找到验证码输入框: {{selector}}')")
            action_codes.append(f"                                        break")
            action_codes.append(f"                            except:")
            action_codes.append(f"                                continue")
            action_codes.append(f"                        ")
            action_codes.append(f"                        if captcha_input and await captcha_input.is_visible(timeout=1000):")
            action_codes.append(f"                            await captcha_input.fill(captcha_text)")
            action_codes.append(f"                            print('验证码已填写')")
            action_codes.append(f"                except Exception as e:")
            action_codes.append(f"                    print(f'验证码处理失败: {{e}}')")
